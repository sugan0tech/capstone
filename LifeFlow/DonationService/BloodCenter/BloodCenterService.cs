...
public class BloodCenterService(
    IBaseRepo<BloodCenter> repo,
    IBaseRepo<DonationSlot.DonationSlot> slotRepo,
    IBaseRepo<UnitBag.UnitBag> unitBagRepo,
    ...
    IMapper mapper) : BaseService<BloodCenter, BloodCenterDto>(repo, mapper), IBloodCenterService
{
    ...
    public async Task<DonationSlotDto> BookAppointment(string centerName, int donorId)
    {
        ...
        var groupedSlots = new Dictionary<TimeSpan, List<DonationSlot.DonationSlot>>();
        for (var time = center.OpenByTime; time < center.CloseByTime; time = time.Add(TimeSpan.FromMinutes(20)))
        {
            ...
            {
                for (int i = groupedSlots[time].Count; i < center.UnitsCapacity; i++)
                {
                    var newSlot = new DonationSlot.DonationSlot
                    {
    ...
    public async Task ProcessSlot(int slotId, SlotStatus status)
    {
        ...
        switch (status)
        {
            case SlotStatus.BloodAccepted:
                var rbcUnit = new UnitBag.UnitBag
                {
                ...
                var plateletUnit = new UnitBag.UnitBag
                {
                    Type = donor.BloodAntigenType,
                    BloodSubtype = donor.BloodSubtype,
                ...
                var plasmaUnit = new UnitBag.UnitBag
                {
                    Type = donor.BloodAntigenType,
                    BloodSubtype = donor.BloodSubtype,
